...
if os.path.exists("celeb_dataset/masked/") == False:
    mkdir("celeb_dataset/masked/")

obj = threading.Semaphore(30)        

def processImage(fileName, folderName):
    mkdir(folderName)
    FaceMasker(fileName, None, True, 'hog', folderName+"/stock.jpg").mask()
    FaceMasker(fileName, DEFAULT_IMAGE_PATH, True, 'hog', folderName+"/type1.jpg").mask()
    FaceMasker(fileName, BLACK_IMAGE_PATH, True, 'hog', folderName+"/type2.jpg").mask()
    FaceMasker(fileName, BLUE_IMAGE_PATH, True, 'hog', folderName+"/type3.jpg").mask()
    FaceMasker(fileName, RED_IMAGE_PATH, True, 'hog', folderName+"/type4.jpg").mask()
    obj.release()

allFiles = listdir("celeb_dataset")
for i, image in enumerate(allFiles):
    print(i, "/" ,len(allFiles))
    fileName = "celeb_dataset/"+image
    folderName = "celeb_dataset/masked/"+image.replace(".jpg","")
    if isfile(fileName) and os.path.exists(folderName) == False:
        obj.acquire()
        threading.Thread(target=processImage, args=(fileName, folderName,)).start()
