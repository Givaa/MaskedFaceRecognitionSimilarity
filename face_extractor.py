...
identities = listdir("dataset_test/m2fred")
for i, identity in enumerate(identities):
    folderName = path.join("dataset_test/m2fred",identity)
    destinationDirectory = path.join("dataset_test/m2fred_face",identity)
    if isdir(destinationDirectory) is False:
        mkdir(destinationDirectory)
    files = listdir(folderName)
    for m, file in enumerate(files):
        if file.find(".jpg") >-1 or file.find(".png") >-1 :
            FaceMasker(path.join("dataset_test/m2fred",identity, file), None, True, 'hog', path.join("dataset_test/m2fred_face",identity, file)).mask()
